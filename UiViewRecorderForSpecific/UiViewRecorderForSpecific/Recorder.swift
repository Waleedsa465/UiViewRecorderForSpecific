...
@objc public class Recorder: NSObject {
    ...
    public func stop(completion: @escaping (_ saveURL: String) -> Void) {
        ...
        if (seconds != nil) {
            guard let firstImageURL = self.imagesArray.first,
                  let firstImage = UIImage(contentsOfFile: firstImageURL.path) else {
                completion("Failed to load images")
                return
            }
            ...
            // Handle the case where the referenceDate is nil
            print("Failed to create video")
            completion("Failed to create video")
        }
